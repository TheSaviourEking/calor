...
export default function WishlistClient({ initialSharedWishlists = [] }: { initialSharedWishlists?: SharedWishlist[] }) {
    const { items, removeItem } = useWishlistStore()
    const { addItem: addToCart } = useCartStore()
    const { formatPrice } = useLocaleStore()
    const [products, setProducts] = useState<WishlistProduct[]>([])
    const [loading, setLoading] = useState(true)
    const [sharedWishlists, setSharedWishlists] = useState<SharedWishlist[]>(initialSharedWishlists)
