...
export default function WellnessClient() {
  ...
  // Fetch session and wellness data
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (sessionData.customer) {
          setProfile(sessionData.customer)
          const customerId = sessionData.customer.id

          // Fetch all wellness data in parallel
          const [streakRes, achievementsRes, challengesRes, checkInRes, profileRes] = await Promise.all([
            fetch(`/api/wellness/streaks?customerId=${customerId}`),
            fetch(`/api/wellness/achievements?customerId=${customerId}`),
            fetch(`/api/wellness/challenges?customerId=${customerId}`),
            fetch(`/api/wellness/checkin?customerId=${customerId}`),
            fetch(`/api/wellness/profile?customerId=${customerId}`),
          ])

          const [streakData, achievementsData, challengesData, checkInData, profileData] = await Promise.all([
            streakRes.json(),
          ...
          // Process achievements with earned status
          if (achievementsData.achievements) {
            setAchievements(achievementsData.achievements.map((a: { earned?: boolean; earnedAt?: string; isEarned?: boolean; userAchievements?: { length: number } }) => ({
              ...a,
              isEarned: a.earned || a.isEarned || (a.userAchievements && a.userAchievements.length > 0) || false,
            })))
          }
          
          // Process challenges with progress
          if (challengesData.challenges) {
            setChallenges(challengesData.challenges.map((c: { userCompletion?: { progress?: number; completed?: boolean; completedAt?: string } }) => ({
              ...c,
              progress: c.userCompletion?.progress || 0,
              isCompleted: c.userCompletion?.completed || false,
              completedAt: c.userCompletion?.completedAt,
            })))
          }
          
          if (checkInData.checkIn) setTodayCheckIn(checkInData.checkIn)
          if (profileData.profile) setWellnessProfile(profileData.profile)

          // Fetch daily rewards
          const rewardsRes = await fetch('/api/wellness/daily-rewards')
          const rewardsData = await rewardsRes.json()
          if (rewardsData.dailyRewards) setDailyRewards(rewardsData.dailyRewards)

          // Fetch couple goals
          const coupleGoalsRes = await fetch(`/api/wellness/couple-goals?customerId=${customerId}`)
          const coupleGoalsData = await coupleGoalsRes.json()
          if (coupleGoalsData.goals) {
            setCoupleGoals(coupleGoalsData.goals)
          }
        }
      } catch (error) {
  ...
  return (
    <AccountLayout 
      ...
      subtitle="Track your journey, earn rewards, and connect with your wellness goals"
      loading={loading}
                          ...
                          {categoryAchievements.map((achievement) => (
                            <div
                              ...
                              className={`p-4 border ${
                                achievement.isEarned ? 'border-green-300 bg-green-50' : 'border-sand'
                              }`}
                            >
                              <div className="flex items-start gap-3">
                                <div className={`w-12 h-12 flex items-center justify-center text-2xl ${
                                  achievement.isEarned ? '' : 'opacity-40 grayscale'
                                }`}>
                                  {achievement.icon}
                                </div>
                                <div className="flex-1">
                                  ...
                                  <div className="h-2 bg-sand">
                                    <div
                                      className={`h-full transition-all ${
                                        goal.completed ? 'bg-green-500' : 'bg-terracotta'
                                      }`}
