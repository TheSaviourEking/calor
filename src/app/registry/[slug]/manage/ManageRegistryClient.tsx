...
export default function ManageRegistryClient() {
              ...
              <div className="flex gap-2">
                {['draft', 'active', 'completed', 'archived'].map((status) => (
                  <button
                    ...
                    className={`px-4 py-2 font-body text-sm capitalize ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative w-12 h-6 transition-colors ${
                      registry.isPublic ? 'bg-terracotta' : 'bg-sand'
                    }`}
                  >
                    <span className={`absolute top-1 w-4 h-4 bg-cream transition-transform ${
                      registry.isPublic ? 'right-1' : 'left-1'
                    }`} />
                  </button>
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={feature.key} className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-12 h-6 transition-colors ${
                        registry[feature.key as keyof Registry] ? 'bg-terracotta' : 'bg-sand'
                      }`}
                    >
                      <span className={`absolute top-1 w-4 h-4 bg-cream transition-transform ${
                        registry[feature.key as keyof Registry] ? 'right-1' : 'left-1'
                      }`} />
                    </button>
                  </div>
                ))}
